...
def get_attendance_answer(student_id, question, pdf_path=None, student_name=None, pdf_pattern=None):
    ...
    # 1️ Extract text from uploaded PDF (if any)
    if pdf_path and os.path.exists(pdf_path):
        context_text += extract_text_from_pdf(pdf_path) + "\n"

    # 2️ Search PDFs folder by naming convention if pdf_path not provided
    elif student_name:
    ...
    # 3️ Fetch attendance info from MongoDB
    ...
    if records:
        for rec in records:
            # Convert MongoDB datetime to readable string
    ...
    # 4️ Call OpenAI / AI model
    try:
        from openai import OpenAI
        client_ai = OpenAI()  # assumes API key is set in environment
        prompt = f"Context:\n{context_text}\n\nQuestion: {question}\nAnswer:"
